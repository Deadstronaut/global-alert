...
- [X] T018 [US3] **Real data restored and RPC live-verified during this phase** (see below) — a genuine bug was found and fixed before any pan/zoom UI check was even reachable; the data-fetch path is confirmed correct at full scale (5,233/5,233 rows, not 1000/5,233). Browser-based pan/zoom responsiveness itself was closed out later, alongside T024 — no stalls with roads (65,070 features) active.
...
- [X] T024 **Closed out via live browser session (test account, 2026-07-27)**: toggled OSM Roads (65,070 features) and Kontur population (Turkey) on together, panned (drag, ~1.3s) and zoomed (wheel ×5, ~1.4s) — no stalls, no dropped frames perceptible, click-to-inspect popup still fires correctly with both layers active. **Found a real, reproducible bug in the process** (exactly what this live check was for): toggling the Kontur population layer produced a genuine server error — `get_dataset_features_geojson` returned Postgres error `57014 canceling statement due to statement timeout` for Turkey's population dataset (20,411 rows, not the raw 457,761 — spec 043's H3-aggregation fix is working, but even the aggregated row count's geometry volume is enough to occasionally exceed the DB's statement timeout). `addExposureLayer()`'s existing silent-failure convention degraded gracefully (layer just didn't render, no crash) — but the underlying timeout is a real, unresolved issue, not a one-off. **Not fixed in this pass** — flagging as a new follow-up (query/geometry-payload optimization, or a longer statement_timeout specifically for this RPC) rather than fixing blind under time pressure; needs its own investigation.
...
- [X] T026 [P] **Live RTL check closed out** (same session as T024): switched the app to Arabic via Settings → Dil → AR, confirmed `document.documentElement` got `dir="rtl"`/`lang="ar"`, and the exposure-layer panel fully mirrored and translated (dataset names, feature counts, checkboxes on the correct side) with no visible overlap/breakage. (Noted in passing, out of scope: the disaster-severity legend box did not translate under AR — pre-existing, unrelated to this feature's own i18n keys, not investigated further here.)
