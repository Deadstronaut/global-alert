...
- [~] T027 [P] **Blocked on T018** — verifying `writeExposureDataset`'s supersession requires actually running `import-osm-roads` twice against a live country with real data, which requires Overpass to be reachable from the deployed Edge Function. Cannot be completed until T018 clears. The supersession logic itself is shared/generalized from Kontur's already-proven `supersedeExposureDataset.ts` (T009/T010), so the mechanism is not new/unverified — only the roads-specific live run is pending.
