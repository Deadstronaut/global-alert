/**
 * Static ISO 3166-1 alpha-2 <-> alpha-3 mapping. This is universal
 * reference data (identical for every deployment/country, like
 * src/data/boundaries's countries.json keying), NOT per-deployment
 * configuration — safe to bundle in shared code, unlike a specific
 * country's dataset references (which live in the DB, never in source).
 *
 * Used by resolveHdxCountryDataset.ts to convert this system's ISO2
 * country_code (e.g. 'TR') to the ISO3 lowercase form HDX's `groups`
 * filter expects (e.g. 'tur').
 */
...
export function iso2ToIso3Lower(iso2: string): string | null {
