...
- [X] T031 Code-review fix-up (2026-08-16): a manual-QA code pass found `submitDraft()` in `CapView.vue` silently no-opped on an empty `title` (`if (!form.value.title.trim()) return`) — the submit button went disabled but no error text ever rendered, unlike the sibling `radiusInvalid` check right below it. Fixed by setting `error.value = t('cap.form.titleRequired')` on the same guard (rendered by the existing `.form-error` element) and clearing `error.value` when a fresh form is opened (`startBlankDraft`/`startFromEvent`) so a stale message can't linger. Added the `cap.form.titleRequired` key to all 7 locales. No DB migration needed (frontend-only). Covered by the existing `npm test` full-suite run (275/275 passed); no new automated test added for this specific string since it mirrors the existing `radiusInvalid` pattern 1:1.
