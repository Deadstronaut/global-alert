...
const { t } = useI18n()
const EARTH_ROTATION_DISTANCE_PX = 400
const randomStartX = Math.floor(Math.random() * EARTH_ROTATION_DISTANCE_PX)
const earthStyle = {
  '--earth-start-x': `${randomStartX}px`,
}
  ...
  <transition name="fade-out">
    <div v-if="visible" class="loading-screen">
      <div class="loading-content">
        <div class="section-banner" :style="earthStyle">
...
.loading-screen {
  ...
  background: radial-gradient(circle at 50% 30%, #0e1a2e 0%, #05070e 60%, #020308 100%);
  display: flex;
  align-items: center;
...
@keyframes earthRotate {
  0% {
    background-position: var(--earth-start-x, 0px) 0;
  }
  100% {
    background-position: calc(var(--earth-start-x, 0px) + 400px) 0;
  }
}
