...
- [ ] T035b **Live-testing finding**: the one real `exposure_datasets` row in the shared dev instance ("Nüfus datası") has `country_code: null` and none of its `exposure_features` rows have `admin_boundary_code` set (0 of N) — its province name lives in `properties.il` (e.g. "Istanbul", "Ankara") instead, which happens to exactly match `country_boundaries`'s TR `shapeName` values for the provinces checked. `save_risk_indicator` will reject this dataset until `country_code` is set, and `compute_risk_category_score`'s join will find nothing until `admin_boundary_code` is backfilled from `properties.il`. This is a live-data gap, not a code defect — needs a decision from the user on how to backfill (one-time SQL `UPDATE` vs. re-upload) before US1/US2 can be demonstrated end-to-end against real data.
...
- [ ] T036 Update `docs/mhews-datalist-partner-questions.md` or a follow-on partner note if this implementation resolves or reframes open question C.2 (INFORM methodology adoption) — cross-check against whatever UNDP has responded with by this point
