...
- [X] T004 [US1] Implement `fetch-live-flights` edge function in `supabase/functions/fetch-live-flights/index.ts`: call OpenSky `GET https://opensky-network.org/api/states/all`, map response rows to the `AircraftState` shape from data-model.md, filter out `onGround: true` and null-position rows, cache the mapped result in a module-level variable with a 10-minute TTL (contracts/fetch-live-flights.md), return the `FlightsLayerResponse` envelope (`fetchedAt`, `stale`, `states`) with existing `corsHeaders` from `../shared/cors.ts`
- [X] T005 [US1] Handle upstream fetch failure in `fetch-live-flights/index.ts`: on error, return the last cached snapshot with `stale: true`, or `{ states: [], stale: true }` if there is no prior cache yet (contracts/fetch-live-flights.md "no data available" case)
- [X] T006 [US1] In `src/components/GlobeView.vue`, add a `flightsData` ref and a `fetchFlights()` function that calls the edge function, stores the response, and is invoked on an interval (~60s) only while `uiStore.showFlights` is true; clear the interval when the layer is toggled off or on unmount (mirror the existing `sunTimer`/`timelinePlayTimer` interval-lifecycle pattern already in this file)
...
- [ ] T019 Follow quickstart.md's validation steps end-to-end — backend cache/edge-function steps not yet run against a live Supabase instance in this session; frontend steps blocked by the app's login gate (no test credentials in this session). Dev-server smoke test (page load, no console errors) passed for both the pre- and post-Phase-5 states.
