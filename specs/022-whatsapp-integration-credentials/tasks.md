...
- [X] T018 **Kapsam dışı kaldı (2026-07-15)**: bu görevin doğrulamak istediği `whatsapp_integration_settings` tablosu hiç canlıya alınmadı — spec 025'in "hard cutover" migration'ı bu tabloyu ve `save_whatsapp_credentials()` fonksiyonunu, WhatsApp'a özel sabit alanlar yerine genel `integration_types` registry'sine dönüştürdü. REST API ile doğrulandı: `whatsapp_integration_settings` production'da mevcut değil (`PGRST205` — tablo yok), `integration_types` mevcut ve sorgulanabiliyor. Bu spec'in orijinal quickstart senaryoları artık spec 025'in (zaten %100 tamamlanmış ve canlıda) kendi kapsamıyla karşılanıyor — ayrı bir doğrulama gerekmiyor.
