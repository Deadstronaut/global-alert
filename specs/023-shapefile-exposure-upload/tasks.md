...
- [ ] T011 Manually validate quickstart.md Scenarios 1–5 (valid Shapefile upload, zero-regression GeoJSON upload, non-WGS84 rejection, malformed-bundle rejection, unsupported-file-type rejection) — **pending**: this requires a real browser session with actual test Shapefile fixtures (WGS84 and non-WGS84 `.zip` bundles) against the live app; the assistant verified code-level correctness (unit tests, clean build with `shpjs` bundled) but has not exercised the live upload UI end-to-end — the user should run through these scenarios in the browser
