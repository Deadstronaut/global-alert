...
- [X] T017 [US1] **Closed out**: added `supabase/migrations/20260727060000_hydro_exposure_import_cron.sql` — `trigger_hydrorivers_import()`, direct copy of `20260718130000_osm_roads_import_cron.sql`'s per-country `net.http_post` loop shape, scheduled monthly (`0 9 1 * *`, matching this source's `poll_interval_seconds=2592000`) as `import-hydrorivers-monthly`. Applied to the linked remote project; confirmed live via `cron.job`. **Found and fixed a real bug while writing this**: the loop pattern (`SELECT country_code FROM country_boundaries`) assumes one row per country — no longer true since `20260727040000_country_boundaries_level.sql` widened the PK to `(country_code, level)`. Used `SELECT DISTINCT country_code` here, and separately patched the same latent bug in the two pre-existing cron triggers that share this pattern (`trigger_osm_roads_import`, `trigger_osm_buildings_import`) via `20260727061000_fix_country_boundaries_level_duplicate_cron.sql` — harmless today (every country still has exactly one boundary row) but would have silently double-triggered imports the moment a country got a second level uploaded.
...
- [X] T022 [US2] **Closed out**: added `trigger_hydrobasins_import()` in the same migration as T017, scheduled monthly (`0 10 1 * *`) as `import-hydrobasins-monthly`, applied and confirmed live.
