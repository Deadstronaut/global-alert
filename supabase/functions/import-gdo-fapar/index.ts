/**
 * Edge Function: import-gdo-fapar
 * Source: Global Drought Observatory, fAPAR Anomaly VIIRS (fpanv) — spec
 * 047 pivot, see gdoAnomalyFetch.ts's header comment.
...
import { fetchGdoAnomaly, GDO_FAPAR_ANOMALY_CONFIG } from '../shared/gdoAnomalyFetch.ts'
...
Deno.serve(async (req) => {
  ...
  const sourceId = await resolveSourceId('vegetation_anomaly', 'GDO fAPAR Anomaly (VIIRS)')
  ...
  try {
    countryFeatures = await fetchGdoAnomaly(GDO_FAPAR_ANOMALY_CONFIG, servedCountryCodes)
  } catch (e) {
  ...
  for (const countryCode of servedCountryCodes) {
    ...
    const { featureCount } = await writeExposureDataset(
      GDO_FAPAR_ANOMALY_CONFIG.sourceName,
      countryCode,
      GDO_FAPAR_ANOMALY_CONFIG.metricPropertyName,
      features,
      GDO_FAPAR_ANOMALY_CONFIG.sourceMetadata,
    )
