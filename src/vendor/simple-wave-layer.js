/**
 * Animated wave particle layer — forked from simple-wind-layer.js
 ...
 * file's own header for the full rationale). WAVE_SPEED_CEILING (12, not
 * wind's 40) matches wave_vector.py's own synthetic vector: its magnitude
 * is significant wave HEIGHT in meters (not a real velocity — see that
 * file's docstring), and storm-sea heights rarely exceed ~12-15m, so
 * reusing wind's 40 m/s ceiling made most real values normalize far below
 * 1 (live-testing finding, 2026-08-06, alongside the same issue confirmed
 * for ocean_current).
 *
 * NOTE: waves are still rendered with the exact same flowing-streamline
 * look as wind/currents for now — the user has flagged that waves should
 * eventually look visually distinct (short, direction-indicating marks,
 * not long flowing lines, since a wave's own motion isn't a flowing
 * current) but asked to fix Currents first; this file is the isolated
 * place that follow-up work should land without touching wind or currents.
 */
...
// (wave_vector.py's synthetic "height" vector, ~0-12, vs. ocean_current's
// real ~0-2 m/s) — at wind's original 0.4 factor this moved wave particles
// a lot each frame (flowing lines), while ocean_current barely moved
// (near-static dots). Live-testing finding, 2026-08-06: the two engines'
// motion scale should be swapped so waves get the dot-like look currents
// had and vice versa. 0.0667 = 0.4 * (2/12), i.e. scaled down by the same
// ratio current's own typical magnitude has under wave's.
const SPEED_FACTOR = 0.0667
const TRAIL_LENGTH = 18
const WAVE_SPEED_CEILING = 12 // meters (significant wave height) — see header

// "Width via many thin parallel strands," not one fat line/point —
// 2026-08-06 follow-up ask, after a first attempt (blowing up trailThickness/
// gl_PointSize 8x) turned out to be the wrong mechanism: "iz kalınlığı
// değil... iz kalınlığı 1.6 pikselde düşün ama bir dalga için yan yana 8
// tane iz attığını düşün, o zaman dalga gibi bir resim oluşuyor" — each
// particle's trail is drawn STRAND_COUNT times, each copy offset a little
// further perpendicular to the particle's CURRENT direction of travel, all
// still thin (this.trailThickness stays small). Overlapping semi-transparent
// strands read as a solid "brush stroke" ribbon, which is what actually
// produces a wave-shaped mark instead of a single traveling dot.
// Was 20, corrected down to 8 (2026-08-06 follow-up: "ben sana 8 kat
// dedim... 8 yan yana olsun... mevcut koyduğunun yarıya indir").
const STRAND_COUNT = 8
const STRAND_SPACING_PX = 1.6 // on-screen pixel gap between adjacent strands, zoom-compensated in render() below
...
function compileShader(gl, type, source) {
  ...
  if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
    ...
    throw new Error(`Wave layer shader compile error: ${info}`)
  }
  return shader
}
...
function linkProgram(gl, vertexSource, fragmentSource) {
  ...
  if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
    ...
    throw new Error(`Wave layer program link error: ${info}`)
  }
  return program
}
...
export class SimpleWaveLayer {
  /**
   * @param {string} id
   * @param {{ textureUrl: string, bounds: [number, number, number, number], dataRange: [[number,number],[number,number]], particleCount?: number, speedMultiplier?: number, trailLength?: number, trailThickness?: number }} options
  ...
  onAdd(map, gl) {
    ...
    image.onerror = (e) => {
      console.warn(`[SimpleWaveLayer] Failed to load texture for ${this.id}`, e)
    }
  ...
  /** Samples the wave texture at a given lng/lat, returns [u, v, speedNormalized] or null if out of bounds. */
  _sampleWave(lng, lat) {
    ...
    const speedNormalized = Math.min(speed / WAVE_SPEED_CEILING, 1)
  ...
  render(gl, options) {
    ...
    const maxVertices = this.particles.length * (trailLength - 1) * 2 * STRAND_COUNT
    const { positions, speeds, alphas } = this._ensureBuffers(maxVertices)
    let vIdx = 0

    // Mercator x/y (what `positions` stores) span the whole world in
    // [0,1] — one unit = the full 360° world's worth of screen pixels at
    // the map's current zoom (standard slippy-map convention: world size
    // in pixels = 256 * 2^zoom). Converting STRAND_SPACING_PX (a fixed
    // on-screen pixel gap) into that same [0,1] unit keeps the strands'
    // visual spacing constant in screen pixels regardless of zoom level,
    // matching how gl_PointSize (also screen-pixel-based) already behaves.
    const worldPixelSize = 256 * Math.pow(2, this.map.getZoom())
    const strandSpacing = STRAND_SPACING_PX / worldPixelSize

    for (let i = 0; i < this.particles.length; i++) {
      let p = this.particles[i]

      if (advect) {
        const sample = this._sampleWave(p.lng, p.lat)
        p.age += 1
      ...
      // Perpendicular unit vector computed ONCE per particle, from its
      // newest segment (current direction of travel) — reused for every
      // history segment's strand offsets below, instead of recomputing it
      // per-segment. Live-testing finding, 2026-08-06: a per-segment
      // perpendicular kinks at every history point along a curving path,
      // since each segment's own direction differs slightly — the strand
      // ribbon visibly breaks into disconnected rectangular blocks instead
      // of one continuous stroke ("segmenti kesmişsin... blok blok
      // görünüyor"). A single, particle-wide perpendicular keeps the whole
      // ribbon's cross-section consistent, reading as one continuous shape.
      let px = 1
      let py = 0
      if (p.history.length > 1) {
        const [nlng, nlat] = p.history[0]
        const [olng, olat] = p.history[1]
        const [nmx, nmy] = lngLatToMercator(nlng, nlat)
        const [omx, omy] = lngLatToMercator(olng, olat)
        const hdx = nmx - omx
        const hdy = nmy - omy
        const headLen = Math.hypot(hdx, hdy)
        if (headLen > 1e-12) {
          px = -hdy / headLen
          py = hdx / headLen
        }
      ...
      for (let h = 0; h < p.history.length - 1; h++) {
        ...
        for (let s = 0; s < STRAND_COUNT; s++) {
          const offset = (s - (STRAND_COUNT - 1) / 2) * strandSpacing
          const ox = px * offset
          const oy = py * offset
          positions[vIdx * 2] = mx0 + ox
          positions[vIdx * 2 + 1] = my0 + oy
          speeds[vIdx] = p.speedNormalized
          alphas[vIdx] = fade * 0.25 * this.opacity
          vIdx++
          positions[vIdx * 2] = mx1 + ox
          positions[vIdx * 2 + 1] = my1 + oy
          speeds[vIdx] = p.speedNormalized
