...
- [ ] T026 [US1] Confirm the seed migration's `endpoint_url` for Kontur is appropriate: `data_sources.endpoint_url` points at HDX's base API (`data.humdata.org/api/3/action/`) since the actual per-country dataset reference lives in `population_source_country_datasets`, not this column — already set correctly in the migration (T003), this task is a final sanity check, not new work.
- [ ] T026a [US1] Run `resolveHdxCountryDataset('kontur', 'tur')` (T005b) for **Turkey** — this system's own reference/test deployment (per project convention: we have direct access to verify Turkish data, unlike third-party example countries in client-provided materials) — as the first real-world proof this resolution mechanism works end-to-end. If it returns `null`, add the row manually using the dataset ID confirmed live in research.md §4 (Kontur's Turkey package is `kontur-population-turkiye`) rather than leaving Turkey unconfigured. Document in this task's commit/PR that every additional country deployment needs its own resolution run (or manual fallback) during onboarding — a recurring per-country onboarding step, not a one-time migration or a code change. No country's `dataset_reference` value may ever appear inside `.ts` source files — DB rows only.
- [ ] T027 [US1] Verify (manual, per quickstart.md §2) the Impact Analysis Wizard's asset-layer selection step lists auto-imported population datasets alongside manually uploaded ones, labeled with their `source_name` — confirm no code change is needed there (existing query already selects all `exposure_datasets` rows for the caller's country); if labeling is missing, add minimal display of `source_name` in that step's dataset list (locate via `grep -r "exposure_datasets" src/`).
...
- [ ] T030 [US2] Verify (manual) that adding a `population` `hazard_types` row does NOT surface "Population" as a selectable option in hazard-specific UI that assumes alertable hazards — check `HazardTaxonomyPanel.vue`, `HazardEncyclopediaPanel.vue`, `HazardThresholdEditor.vue`, and CAP alert-authoring's hazard picker; if any of these show it inappropriately, scope it out via that component's own existing category/filter mechanism (do not remove the `hazard_types` row — filter at the display layer, consistent with how `SOURCE_SUPPORTED_HAZARDS` already filters per-component).
- [ ] T031 [P] [US2] Add a Deno test verifying `recordFetchOutcome`/`computeNextState` (existing, unmodified) behave identically for `hazard_type: 'population'` rows as for any other value — i.e. confirm no hidden hazard-type-specific branching exists anywhere in `sourceHealth.ts` (should be a no-op confirmation test, not new logic).
- [ ] T032 [US2] Manual verification (quickstart.md §3): break Kontur's `endpoint_url`, trigger its import function repeatedly, confirm `health_state` transitions to `degraded`/`down` in the Sources tab and a `source_state_transitions` row is written; restore and confirm recovery to `healthy`.
...
- [ ] T033 [US3] Wire `validatePopulationRecord` rejections in `import-kontur-population/index.ts` to call `logRejectedPayload(sourceId, 'population', reason, record)` for every invalid record (should already be exercised if T023 correctly used T006 — this task is the explicit verification pass).
- [ ] T034 [P] [US3] Add an integration-style test (or extend T007) proving a batch of exclusively invalid records still results in `recordFetchOutcome(sourceId, 'success', ...)` for that import run — per research.md §3's "zero valid records is not a failure" convention.
- [ ] T035 [US3] Manual verification (quickstart.md §4): confirm rejected records are visible via the existing rejected-payloads admin view/query, filtered to `hazard_type: 'population'`, with readable reasons.
...
- [ ] T036 [P] Manual verification (quickstart.md §5): run Kontur's import twice for the same country, confirm exactly one `exposure_datasets` row remains for that `(source_name, country_code)` pair afterward, and that the country is never left with zero population features between the two runs.
- [ ] T037 [P] Add Kontur's polling entry to `src/services/api/config.js`'s `EDGE_FUNCTIONS`/`POLLING_INTERVALS` (interval matching T003's `poll_interval_seconds`), only if this app's existing client-driven trigger pattern is confirmed (during T027) to be how other admin-manageable sources are actually invoked in production, rather than a manual-only "run now" action — verify against the existing Sources tab's actual trigger mechanism first, since research.md §5 assumed client-driven polling but this must be confirmed against current `src/services/api/` code, not just `config.js`'s existing entries for hazard sources.
- [ ] T038 Run full quickstart.md validation end-to-end (all sections, including 1a) and record results.
- [ ] T039 [P] Run `deno test --no-check --allow-net --allow-env supabase/functions/shared/` and confirm no regressions in the pre-existing suite.
- [ ] T040 [P] Run `deno check` across all new/modified files listed in plan.md's Project Structure section and confirm no new type errors beyond the pre-existing `esm.sh`/strict-catch baseline noted in feature 001's tasks.md.
