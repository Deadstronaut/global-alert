...
- [X] T019 Code-review fix-up (2026-08-16): a manual-QA code pass found `get_access_review()` never returned `locked_until` in its `RETURNS TABLE`, even after the T017 fix-up — `UsersPanel.vue`'s `isLocked()` reads `row?.locked_until` from exactly this RPC's result, so it was always `undefined`/`false` for every profile: the 🔒 lock badge (T013) never rendered, the "Kilidi Aç" unlock button never appeared even for genuinely locked accounts, and the CSV/JSON access-review export's `locked` column (FR-003) was always `false` regardless of real state. Fixed via `supabase/migrations/20260816090000_fix_access_review_locked_until.sql`, which re-`CREATE OR REPLACE`s `get_access_review()` adding `p.locked_until` to both the `RETURNS TABLE` and the `SELECT` list — same fix-up pattern as T017. No frontend change needed (`UsersPanel.vue` already read the field defensively). Migration applied by the user; `npm test` full-suite (275/275) passed after the change. Live re-verification of the badge/button/export against a genuinely locked test account is still the user's to do, same caveat as T017.
